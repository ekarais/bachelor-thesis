...
3. Need to definitively understand if softmax is implemented correctly.
'''
...
model = Autoencoder(params['INPUT_DIM']).to(device)
wandb.watch(model)
...
def auto_train():
    ...
    for i, x in enumerate(train_iterator):
        ...
        # reconstruction loss
        loss = F.binary_cross_entropy(x_sample, x, reduction='sum')
...
def auto_test(iterator):
    ...
    for i, x in enumerate(iterator):
        # reshape the data into [batch_size, 9*336]
        x = x.view(-1, 9 * 336).float()
        x = x.to(device)
...
for e in range(params['N_EPOCHS']):
    ...
    '''
    train_loss = auto_train() / len(training_set)
    val_loss = auto_test(validation_iterator) / len(validation_set)
    test_loss = auto_test(test_iterator) / len(test_set)
